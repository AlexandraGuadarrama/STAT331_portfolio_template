...
fish <- fish |>
  mutate(length_cm = replace_values_outside_bounds(length_cm, 5, 80),
         weight = replace_values_outside_bounds(weight, 10, 4000))
...
#| label: wd-6-semi
# Lab 5
# Modified Lab 5 using a semi_join to pull interviews where
# there could be a potential match
interview |>
  semi_join(person, 
...
#| label: wd-6-anti
# Lab 5
# Modified Lab 5 code to demonstate how anti_join can be
# used to pull names of suspects that have no matching interview
final_suspect <- interview |>
  anti_join(potential_suspect, 
            by = join_by("person_id" == "id")) |>
  pull(name)
```
