...
```{r}
#| label: dvs-3-2
# Lab 4 Challenge, Analysis Question 2
...
# and calculate numerical summaries. This would take the mean across 
# all variables. (idk)

rate_my_prof <- teacher_evals_clean |>
  filter(seniority == 1) |>
  group_by(teacher_id) |>
  summarize(across(c(avg_course_fail, SET_score_avg, course_id), 
                    ~ mean(.x, na.rm = TRUE)),
            courses_taught = n_distinct(course_id),
            .groups = "drop")
```
